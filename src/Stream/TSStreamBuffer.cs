...
namespace BDInfo
{
    public class TSStreamBuffer
    {
        ...
        public void Add(byte[] buffer, int offset, int length)
        {
            ...
            if (_bufferLength + length >= _buffer.Length) length = _buffer.Length - _bufferLength;
        ...
        {
            if (_stream.Position + bytes >= _bufferLength) return null;
        ...
        {
            if (_skipBits > 0) BSSkipBits(8 - _skipBits);
        }
        ...
        public uint ReadExp()
        {
            ...
            while (DataBitStreamRemain() > 0 && !ReadBool()) leadingZeroes++;
