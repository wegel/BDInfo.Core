...
namespace BDInfo
{
    public class TSCodecAC3: ITSAudioCodec
    {
        ...
        public void Scan(TSAudioStream stream, TSStreamBuffer buffer, ref string tag, long? bitrate)
        {
            ...
            if (sync == null || sync[0] != 0x0B || sync[1] != 0x77) return;
            ...
            if (bsid <= 10)
            {
                ...
                if ((channelMode & 0x1) > 0 && channelMode != 0x1) buffer.BSSkipBits(2);
                if ((channelMode & 0x4) > 0) buffer.BSSkipBits(2);
                if (channelMode == 0x2)
                {
                    var dsurmod = buffer.ReadBits2(2);
                    if (dsurmod == 0x2) stream.AudioMode = TSAudioMode.Surround;
                }

                lfeOn = buffer.ReadBits2(1);
                dialNorm = buffer.ReadBits2(5);
                if (buffer.ReadBool()) buffer.BSSkipBits(8);
                if (buffer.ReadBool()) buffer.BSSkipBits(8);
                if (buffer.ReadBool()) buffer.BSSkipBits(7);
                if (channelMode == 0)
                {
                    buffer.BSSkipBits(5);
                    if (buffer.ReadBool()) buffer.BSSkipBits(8);
                ...
                if (bsid == 6)
                {
                    if (buffer.ReadBool()) buffer.BSSkipBits(14);
                    if (buffer.ReadBool())
                    {
                        ...
                        if (dsurexmod == 2) stream.AudioMode = TSAudioMode.Extended;
                    }
                }
            }
            else
            {
                ...
                if (emdfFound)
                {
                    ...
                    if (emdfVersion == 3) emdfVersion += buffer.ReadBits2(2);
                    ...
                    else
                    {
                        ...
                        if (temp == 0x7) buffer.BSSkipBits(2); //skip 3 bits

                        var emdfPayloadID = buffer.ReadBits2(5);

                        if (emdfPayloadID > 0 && emdfPayloadID < 16)
                        {
                            if (emdfPayloadID == 0x1F) buffer.BSSkipBits(5); //skip 5 bits
                        ...
                        if (buffer.Position < buffer.Length && emdfPayloadID == 14)
                        {
                            ...
                            if (jocNumObjectsBits > 0) stream.HasExtensions = true;
                        }
                    }
                }
            }

            if (channelMode < 8 && stream.ChannelCount == 0) stream.ChannelCount = AC3Channels[channelMode];
            ...
            if (bsid <= 10)
            {
                ...
                if (fSize < 19) stream.BitRate = AC3Bitrate[fSize] * 1000;
            }
            else
            {
                stream.BitRate = (long) (4.0 * frameSize * stream.SampleRate / (numBlocks * 256));
                if (stream.CoreStream != null) stream.BitRate += stream.CoreStream.BitRate;
            }
            ...
            if (stream.StreamType != TSStreamType.AC3_PLUS_SECONDARY_AUDIO)
            {
                ...
                else if (stream.StreamType == TSStreamType.AC3_PLUS_AUDIO && secondFrame) stream.DialNorm = (int) dialNormExt * -1;
            }
        ...
        private static void EmdfPayloadConfig(TSStreamBuffer buffer)
        {
            ...
            if (sampleOffsetE) buffer.BSSkipBits(12);
            ...
            if (buffer.ReadBool()) buffer.BSSkipBits(8); // reserved
