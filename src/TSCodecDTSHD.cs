namespace BDInfo
{
    public abstract class TSCodecDTSHD
    {
        ...
        public static void Scan(TSAudioStream stream, TSStreamBuffer buffer, long bitrate, ref string tag)
        {
            ...
            {
                tag = "CORE";
                if (stream.CoreStream == null) stream.CoreStream = new TSAudioStream {StreamType = TSStreamType.DTS_AUDIO};
            ...
            if (bStaticFieldsPresent)
            {
                ...
                if (buffer.ReadBool()) buffer.BSSkipBits(36);
                ...
                for (var i = 0; i < nuNumAudioPresent; i++) nuActiveExSsMask[i] = buffer.ReadBits4((int) (nuSubStreamIndex + 1)); //?
                ...
                if (buffer.ReadBool())
                {
                    ...
                    for (var i = 0; i < nuNumMixOutConfigs; i++) nuMixOutChMask[i] = buffer.ReadBits4(nuBits4MixOutMask);
                }
            }
            ...
            for (var i = 0; i < nuNumAssets; i++)
            {
                ...
                if (bStaticFieldsPresent)
                {
                    if (buffer.ReadBool()) buffer.BSSkipBits(4);
                    if (buffer.ReadBool()) buffer.BSSkipBits(24);
                    if (buffer.ReadBool())
                    {
                        var nuInfoTextByteSize = buffer.ReadBits2(10) + 1;
                        var infoText = new ushort[nuInfoTextByteSize];
                        for (var j = 0; j < nuInfoTextByteSize; j++) infoText[j] = buffer.ReadBits2(8);
                    }
                    ...
                    if (buffer.ReadBool())
                    {
                        if (nuTotalNumChs > 2) buffer.BSSkipBits(1);
                        if (nuTotalNumChs > 6) buffer.BSSkipBits(1);
                        if (buffer.ReadBool())
                        {
                    ...
                    stream.LFE = 0;
                    if ((nuSpkrActivityMask & 0x8) == 0x8) ++stream.LFE;
                    if ((nuSpkrActivityMask & 0x1000) == 0x1000) ++stream.LFE;
            ...
            if (stream.CoreStream != null)
            {
                ...
                if (coreStream.AudioMode == TSAudioMode.Extended && stream.ChannelCount == 5) stream.AudioMode = TSAudioMode.Extended;
                /*
